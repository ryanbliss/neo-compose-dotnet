...
- Fix saves that fail to load with "P75 sparse instance root … is not reachable through a Class member placement" after C# assigns `null` to a List entry, Dictionary entry or Class member that held an instance (for example `inventory.Stacks[0] = null`). The write kept the slot's row id and dropped the instance, but the rows the instance owned stayed saved with nothing linking them. On reload the loader rejected the first such class row. Replacing a row in place now releases the owned rows the new value no longer links, and the replaced instance's sparse overrides, the same way NeoScript already did for a nulled class slot.
...
- Add `TryGetGameObject` to world objects, so gameplay code reaches what a grid renderer drew through the object's data instead of searching the hierarchy by path:
  ...
  A placed object and each composition child answer with their own GameObject. It is registered before the object's spawn hooks run and stays until its despawn hooks return, including when the renderer is cleared, re-rendered, or destroyed. A tile layer link answers with its target layer's Tilemap GameObject, and an object layer link with its layer's root. Links keep answering inside despawn hooks fired by a clear or destroy. A removed object's own carried links stop answering in the write that removes it, since their tiles leave the Tilemap in that write. It returns false when no renderer currently draws the value.
...
- The sort point is live. A `SortPoint` write moves only the pair's two transforms in the same coalesced refresh as other animation writes, and never respawns the object. A variant swap's `SortPoint` applies the same way on a composition child; on a placed object it arrives through the respawn the variant swap already causes. Assigning the object a new sorting group moves the pair to the new group's sort point; assigning null leaves the pair at its last point until the next group. `SortAtRoot` stays as read at spawn. A Position write does not read the sort point.
...
- Remove the overrides stored under a virtual root when the root is removed. A write under a virtual chain stores sparse spine rows and leaves at ids minted from the root's namespace; nothing else owns them, but the root's committed expansion still links them, so reachability kept them alive after the root died. The orphaned spine then replayed without its outer initializers and despawning a destroyed boulder failed with "Sprite has a computed default and cannot be materialized as a literal". Removing a root, by tombstone or direct removal, now removes every row in its footprint in the same store, plus the Session overlay above a removed Save root, along with what those rows own. A Session tombstone over a Save root leaves the Save overrides in place for when it lifts.
- Keep a constructed unordered-list entry through a candidate replay. The allocation sweep proved ownership of an unordered entry by reading its container stamp from the committed stores only, so an entry the replay had just constructed as a Session allocation had no provable parent and was reclaimed; the replayed list came back empty. The lookup now reads Session rows through the candidate's view.
- Verified in Neowyn over the real export: pickaxe hits lower a boulder's Health, swap it Full → Cracked → Broken as it crosses each stage's `MinHealth` (the drawn sprite and the collider follow), keep that state through save and reload (including a stage swap made after the reload), and despawn it at zero with no exceptions logged.
...
- Cut NeoScript interpreter garbage and time on the per-frame paths. Measured on Neowyn's `WorldTime.AddTime` over the real export (Unity profiler, package client): a free-roam tick fell from 19.75 KB and 85 µs to 4.64 KB and 40 µs, a paused tick from 6.02 KB and 23 µs to 1.86 KB and 10 µs, a generated float setter from 3.03 KB and 17 µs to 0.15 KB and 2 µs, and a two-level getter after a write from 10.22 KB and 41 µs to 3.12 KB and 20 µs.
  - A NeoScript field assignment resolved its target member by re-merging the class's whole instance schema on every write (2.8 KB and 12 µs each); it now uses the client's cached per-class surface lookup.
  - Generated scalar setters (`Set` on number, string, bool, vector, colour, file, enum, lookup, dialogue-lookup and decimal nodes) store the leaf in place when the committed row allows it, the same path NeoScript assignments took since the leaf-write change, and build a write plan only when they must. A leaf that a constructed graph read while it expanded (an initializer that copies it) is a replay dependency of that graph, so both paths now leave it to the plan, which re-expands the dependents; the NeoScript path previously stored such a leaf without replaying them.
  - Getter frames execute with the client's shared immediate options, so a nested getter reuses the caller's expression handlers instead of allocating two closures and a context fork per evaluation.
  - The executor's frame loop, pointer evaluation, function-call evaluation and the class-member write target no longer declare lambdas in their hot methods; the C# compiler allocated those closures on every call whether or not the deferred, paused or plan branch ran. Nested-block continuations, action-call frame names, deferred native suspension and the plan-backed write moved into their own methods, and a nested block that settles synchronously returns its result directly.
  - Getter memo read lists and per-row key sets are pooled across the forget/re-record cycle a write causes, `if` conditions evaluate their boolean expression directly instead of wrapping it in a pointer and operation, two-operand numeric arithmetic folds without operand arrays, a static binding read builds its dependency key once per member, a direct `Invoke` reuses its synthetic call-site pointer, the receiver-generic environment cache is created on first use, and each non-generic NSFunction shares one terminal normalizer instead of closing over every invocation's context.
...
- Cut NeoScript per-call overhead. Immediate calls reuse a single client-wide execution option set and its expression handlers instead of building closures and continuation state per call; loop bodies reuse one scope across iterations; fallthrough, break, and continue results are shared singletons; a normalized return value that is already the executed value reuses the execution result; trailing-default filling and diagnostic subjects allocate only on the error path; primitive literal pointers cache their unwrapped value; runtime dispatch caches key on a tuple instead of a concatenated string; and read-only members materialize their declaration default row once per declaration instead of on every read.
- Rebuild the authored ownership map when a value partition loads or unloads. A world grid's Save-declared placed objects are unreachable from the main map until their partition merges, so a prebuilt placement (a boulder placed in the seed) stayed classified as Asset and every write to it failed with "not save-owned" even though the save file held it.
- Memoize top-level NSProperty computes on a row-backed receiver, and keep memoization on under dependency capture (animation segment sources, nested constructors): a hit replays the value ids the evaluation reported, so a capture records the same dependency set either way. Reuse each property node's grid-read recorder across evaluations instead of allocating one per read.
- Replace a bound scalar child at its stable id when NeoScript assigns a field. The write path cloned the parent row twice and recommitted it with a new `updatedAt` on every scalar assignment, which recommitted the whole clock row three times per world-time tick and republished it to every subscriber; generated setters never did this.
- Store writable-value subscriptions as a handler list per value id and publish over a pooled snapshot. A multicast delegate copied its whole invocation array on every subscribe and unsubscribe, which sixty animation tracks sharing one facing dependency turned into O(n²) churn on every re-resolution. Reentrancy semantics are unchanged.
- Cut allocation in the write pipeline: placement validation reuses pooled scratch sets, collects placement parents without an iterator, and caches world-kind lookups per class; lookup caches keep their object and tile layer ids until a structural or partition change, and object-move bookkeeping allocates lazily; commit scratch (changed rows, previous containers) and getter-memo invalidation scratch are pooled; a generated setter allocates its write plan only on the branch that commits; delegate call frames are tuples, with member names spelled out only in cycle and depth error text.
- Return a localized string member's template at a bare member read instead of formatting it. Formatting a template that expects arguments logged a warning on every read and returned the template anyway; consumers that hold arguments (NeoDialogue) still format explicitly.
- Check the generated-class wrapper cache before building a replacement node tree when an NSProperty returns a class value, so a read that repeats every frame (the equipped item, the placement target) allocates nothing after its first resolution.
...
- Give placement its own write API. An object's Position and a tile's Cell are the only scalar members the grid indexes read, so generated setters and NeoScript assignments to those members now call `NeoMemberClassWritable.SetPlacementValue` (through the new `NeoGeneratedTypesSupport.SetPlacementVector3`/`SetPlacementVector2Int` funnels and their `OrClear` variants) instead of the plain scalar setter. An object's Position is validated against the loaded grid indexes and stored in place: a move that stays in its cell allocates nothing beyond the row, a move that changes cell patches only the moved footprint and its projected tiles, and a collision or non-finite position leaves the store and every index untouched. A tile's Cell still commits through the plan, which rebuilds the layer.
- The plain leaf write no longer walks the placement-parent index looking for a grid, tile, object, layer or link ancestor. Scalars beneath a world placement that the indexes do not read (an object's Enabled, a layer link's order) store in place like any other leaf.
- The write plan drops its runtime-leaf branch. A Position written as a raw row rather than through the placement API — `SetWritableValue`/`SetWritableValues`, and the bound vector-component wrappers that commit through them (`obj.Position.y = 1f`) — validates structurally and rebuilds the affected layers, as it did before 0.39.7.
- Animated `Position`/`Cell` writes route through the placement API. A clip applies each animated leaf on its owner node, so a track on an object's Position (written whole, or composed from `~partial` field writes) now patches the grid indexes, raises the same `PositionsOnly` change a generated setter does and drops the grid-dependent getter memo, instead of storing a scalar the indexes never see.
- Generated code must be regenerated: Neo Compose emits the placement funnels for `Position` on the `NeoObjectBase` family and `Cell` on the `NeoTile`/`NeoPlacementTile` families, and the codegen contract hash changes with it.
...
- Replay independent nested leaf constructors in their original ID namespace when equipment changes. Keep unchanged character layers and wrapper identities; preserve call-site fields, saved overrides, constructor evaluation order, and conservative enclosing replay for dependent or side-effecting constructions. Class identity/field dependencies no longer treat ownership/type metadata as whole-object reads. Install each prepared nested expansion once during replay.
...
- Keep unordered list mutations visible to aliases, counts, and foreach loops, including whole-list assignment.
...
- Reduce project JSON deserialization allocations and parse resource-backed project JSON off the main thread during play on supported platforms.
- Speed up client preparation with indexed value inference and wrapper cleanup.
- Render world grids with an 8 ms cooperative frame budget by default, retaining explicit tile and object limits and avoiding unnecessary frame waits.
...
- **Breaking:** the project export schema version is now 29 and consolidates
  ...
  enum values, and unknown ordinals so mismatched exports fail before runtime
  construction. An older SDK that bypassed the schema-version gate would
