...
Full SDK EditMode suite: 2,357 passed, 0 failed, 11 skipped. Focused tests cover exact alias refresh, scalar live reads, delegate identity, direct versus stored patterns, query ordering/dependencies, no generated-wrapper requirement, multi-link insertion, occupied-cell rejection and unrelated index retention. The final full Neowyn suite passed 349/350, including both benchmarks and the farming/furniture UI. The remaining startup test measured a 536.5 ms frame gap against its unchanged 500 ms limit (#183). A focused character/inventory/sandbox sequence passed 15/15. Earlier full runs intermittently failed zero-movement or bed-contact assertions; their later success does not prove the cause. The benchmark keeps movement/input/collision diagnostics, and the bed check now waits for the observable contact menu with a bounded timeout (the final run used zero extra frames). These limitations remain recorded in #184.
